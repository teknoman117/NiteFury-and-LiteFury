...
def mem_test_random(size_in_MiB):
    ...
    start = time.time()
    for page in range(NUM_TRANSFERS):
       os.pwrite(fd_h2c, tx_data[page], (1024**3) + (page * TRANSFER_SIZE));
    end = time.time()
    duration = end-start;

    # Print time
    BPS = TRANSFER_SIZE * NUM_TRANSFERS / (duration);
    print("Sent %d MiB in %.3fs (%.3fMBPS)" % (1, duration, BPS/1000000))

    # Receive from FPGA block RAM
    start = time.time()
    for page in range(NUM_TRANSFERS):
       rx_data.append(os.pread(fd_c2h, TRANSFER_SIZE, (1024**3) + (page * TRANSFER_SIZE)))
    end = time.time()
    duration = end-start;

    # Print time
    BPS = TRANSFER_SIZE * NUM_TRANSFERS / (duration);
    print("Received %d MiB in %.3fs (%.3fMBPS)" % (1, duration, BPS/1000000))
